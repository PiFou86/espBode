...
/* Packet length must be four-bytes aligned. This function generates remaining
    ...
    */
uint8_t getPadding(uint8_t unpadded) { return 3 - (unpadded + 3) % 4; }
...
uint32_t AWGNetwork::receiveRpcPacket(uint8_t **data, WiFiClient client) {
...
void AWGNetwork::sendReadResponse(uint32_t xid, WiFiClient client) {
  uint8_t length =
      sizeof(rcpresp_devReadWrite) + strlen(ID) + getPadding(strlen(ID));
  ...
  response->dataLen =
      strlen((char *)_awgParser->_gReadBuffer) + getPadding(strlen((char *)_awgParser->_gReadBuffer));
  strcpy((char *)(response->data), (char *)_awgParser->_gReadBuffer);
...
uint8_t AWGNetwork::handlePortmap(uint8_t *packet, WiFiClient client) {
  ...
  if (getport->header.procedure != PORTMAP_GETPORT) {
    ...
    fillResponseHeader((uint8_t *)&(response.header), getport->header.xid,
                       sizeof(rpcresp_getport));
...
void AWGNetwork::parseVxiWrite(uint8_t *packet) {
  rcpreq_devReadWrite *request = (rcpreq_devReadWrite *)packet;

  swapEndianess((uint8_t *)&(request->data), request->dataLen);
  _awgParser->handleWriteMsg((char *)&(request->data), request->dataLen);
}

uint8_t AWGNetwork::handleVxi11(uint8_t *packet, WiFiClient client) {
  ...
  switch (header->procedure) {
    case VXI_11_CREATE_LINK:
      ...
      fillResponseHeader((uint8_t *)&(create_response.header), header->xid,
                         sizeof(rpcresp_createLink));
    ...
    case VXI_11_DEV_WRITE:
      ...
      fillResponseHeader((uint8_t *)&(write_response), header->xid,
                         sizeof(rcpresp_devReadWrite));
...
uint8_t AWGNetwork::handlePacket(WiFiClient client) {
  uint8_t retVal = 0;
  rpcreq_header *header;
